...
---
        ...
        Explains how one Gemini API key powers multiple agents through prompt separation, context packets, response schemas, and backend validation.

        ## Role In The Product

        StadiumMind AI is intentionally designed as a multi-role platform rather than a single chatbot. This document defines how the gemini ai overview part of the system should behave, what it depends on, and how an AI coding agent should implement it without guessing missing product decisions.
...
- Translation.
        ...
        - All Gemini calls must go through backend.
- Use JSON schemas for state-changing output.
- Use deterministic fallback for demo.
