...
---
        ...
        Defines state boundaries for match, ticket, journey, dashboard snapshot, simulation, socket events, and auth.

        ## Role In The Product

        StadiumMind AI is intentionally designed as a multi-role platform rather than a single chatbot. This document defines how the frontend state management part of the system should behave, what it depends on, and how an AI coding agent should implement it without guessing missing product decisions.
...
- Auth state.
        ...
        - Use explicit loading and fallback states.
- Avoid global state for transient modals.
